...
class EventRepository {
    ...
    /**
     * Uploads an image Uri to Firebase Storage under 'events' path.
     */
    suspend fun uploadImageToStorage(context: Context, uri: Uri, folder: String = "events"): Result<String> {
        return FirebaseStorageManager.uploadFile(context, uri, folder)
    }

    /**
     * Uploads multiple image Uris to Firebase Storage under 'events' path.
     */
    suspend fun uploadMultipleImagesToStorage(context: Context, uris: List<Uri>, folder: String = "events"): Result<List<String>> {
